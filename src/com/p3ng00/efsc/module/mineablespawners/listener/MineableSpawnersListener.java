...
public class MineableSpawnersListener implements Listener {
    ...
    public void onSpawnerMine(BlockBreakEvent event) {
        ...
        if (block.getType() != Material.SPAWNER || event.isCancelled() || !event.getPlayer().getInventory().getItemInMainHand().containsEnchantment(Enchantment.SILK_TOUCH))
            return;
    ...
    public void onSpawnerPlace(BlockPlaceEvent e) {
        ...
        if (!EFSC.MINEABLE_SPAWNERS.isEnabled() || e.getBlockPlaced().getType() != Material.SPAWNER || e.getItemInHand().getItemMeta() == null || e.getItemInHand().getItemMeta().getDisplayName().equals(""))
            return;
